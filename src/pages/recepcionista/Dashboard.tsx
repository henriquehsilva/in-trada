...
const RecepcionistaDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const carregarEventos = async () => {
      try {
        // Se o recepcionista está vinculado a um evento específico,
        // carrega apenas esse evento
        if (userData?.eventoId) {
          const evento = await obterEventos();
          setEventos(evento.filter(e => e.id === userData.eventoId));
        } else {
          // Caso contrário, carrega todos os eventos (comportamento temporário)
  ...
  if (loading) {
    return (
      <LayoutDefault title="Dashboard do Recepcionista">
  ...
  return (
    <LayoutDefault title="Dashboard do Recepcionista">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-100">
          <h2 className="text-lg font-semibold mb-4">Eventos Disponíveis</h2>
          <div className="space-y-4">
            {eventos.map(evento => (
              <div
                key={evento.id}
                className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition-colors cursor-pointer"
                onClick={() => navigate(`/recepcionista/painel/${evento.id}`)}
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {eventos.map(evento => (
              ...
              >
                ...
                <h3 className="font-medium mb-1">Iniciar Check-in</h3>
            ...
            {eventos.length === 0 && (
              <div className="col-span-2 text-center py-4">
                <p className="text-gray-500">
                  Selecione um evento para ver as ações disponíveis.
                </p>
              </div>
            )}
          </div>
